...
public class PlaylistApiEndpoints : EndpointGroup
{
    ...
    public const int MaxCreateAmount = 4;
    ...
    {
        if (!string.IsNullOrWhiteSpace(body.IconHash) && body.IconHash != "0")
        {
            // playlists only have icons in LBP1 and LBP1-like beta builds, so arbitrarily pass LBP1
            if (body.IconHash.StartsWith('g'))
            {
                if (!guidChecker.IsTextureGuid(TokenGame.LittleBigPlanet1, long.Parse(body.IconHash)))
                {
            ...
            {
                GameAsset? icon = database.GetAssetFromHash(body.IconHash);
    ...
    [DocError(typeof(ApiNotFoundError), ApiNotFoundError.PlaylistMissingErrorWhen)]
    [DocError(typeof(ApiValidationError), ApiValidationError.NoPlaylistDeletionPermissionErrorWhen)]
        ...
        GameUser user, int id)
    {
        ...
        if (user.UserId != playlist.PublisherId) 
            return ApiValidationError.NoPlaylistDeletionPermissionError;
    ...
    [ApiV3Endpoint("playlists/id/{playlistId}/add/level/id/{levelId}", HttpMethods.Post)]
    ...
    [ApiV3Endpoint("playlists/id/{playlistId}/remove/level/id/{levelId}", HttpMethods.Post)]
    ...
    [ApiV3Endpoint("playlists/id/{playlistId}/reorder/levels", HttpMethods.Post)]
    [DocSummary("Reorders the levels of a playlist. The IDs of levels which are not in the playlist will be ignored.")]
    [DocError(typeof(ApiNotFoundError), ApiNotFoundError.PlaylistMissingErrorWhen)]
    [DocError(typeof(ApiValidationError), ApiValidationError.NoPlaylistEditPermissionErrorWhen)]
    public ApiOkResponse ReorderLevelsInPlaylist(RequestContext context, DataContext dataContext,
        GameUser user, ApiLevelListRequest body, int playlistId, int levelId)
    {
        GamePlaylist? playlist = dataContext.Database.GetPlaylistById(playlistId);
        if (playlist == null) return ApiNotFoundError.PlaylistMissingError;
    ...
    [ApiV3Endpoint("playlists/id/{playlistId}/add/playlist/id/{subPlaylistId}", HttpMethods.Post)]
    ...
    [ApiV3Endpoint("playlists/id/{playlistId}/remove/playlist/id/{subPlaylistId}", HttpMethods.Post)]
